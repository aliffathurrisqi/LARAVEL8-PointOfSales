...
@section('contents')
    ...
    <section class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1>Edit Pembelian - {{ $purchases->code }}</h1>
          </div>
          <div class="col-sm-6">
    ...
    <!-- Main content -->
    <form method="POST" action="{{ route('transaction-purchasing-update') }}">
    @csrf
              ...
              <div class="card-header">
                ...
                <div class="row">
                    <div class="form-group col-6">
                        <label>Code : </label>
                        <input type="text" class="form-control" name="code" placeholder="Masukkan Code" value="{{ $purchases->code }}" required>
                    </div>

                    <div class="form-group col-6">
                        <label>Purchase Date : </label>
                        <input type="date" class="form-control" name="purchase_date" value="{{ $purchases->purchase_date->isoFormat("Y-MM-DD") }}" required>
                    </div>
              ...
              <div class="card-body">
                <table id="datatable" class="table table-bordered table-striped">
                  ...
                  <tbody id="newinput">
                    ...
                    @foreach ($purchases->details as $det)
                    <tr id='row'>
                        <td>
                            <select name='product_id[]' class='form-control'>
                                @foreach ($products as $item) <option value="{{ $item->id }}"
                                    @if ($item->id == $det->product_id)
                                    selected
                                    @endif
                        ...
                        <td>
                            <input type='number' min=1 name='quantity[]' placeholder='Masukkan jumlah' class='form-control' value="{{ $det->quantity }}" required>
                        </td>
                        <td>
                            <input type='number' class='form-control' placeholder='Masukkan Harga' name='price[]' min='1' step='any' value="{{ $det->price }}" required>
                        </td>
